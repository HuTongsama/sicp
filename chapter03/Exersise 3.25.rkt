    ...
    (define (insert! keys value)
      (define (make-record keys)
        (if (pair? keys)
            (if (null? (cdr keys))
                (cons (car keys) value)
                (cons (car keys) (make-record (cdr keys))))
            (if (null? keys)
                (list value)
                (cons keys value))))
      (define (iter cur-keys cur-table)
        (let ((subtable (assoc (car cur-keys) (cdr cur-table))))
          (if subtable
              ...
              (set-cdr! cur-table
                        (cons
                         (list (car cur-keys) (make-record (cdr cur-keys)))
                         (cdr cur-table))))))
        (iter keys local-table))
  (define (dispatch m)
    (cond ((eq? m 'lookup-proc) lookup)
          ((eq? m 'insert-proc!) insert!)
          ((eq? m 'show) (lambda ()(display local-table) (newline)))
